# https://www.wikipython.com/tkinter-ttk-tix/gui-demos/a-tkinter-multi-screen-strategy-demo/
#_author_ = "John Oakey"
...
class ScreenNo1:
    def __init__(self, top1):
        ...
        self.greeting1 = Button(frame, text="Click Me", fg="blue", command=self.write_greeting1, font=("arial", 16, "bold"))
...
class ScreenNo2:
    def __init__(self, top1):
        ...
        self.button2 = Button(frame2, text="Close TopLevel 2", fg="maroon", command=top2.destroy, font=("courier", 16, "bold"))
        # could close the whole shebang by using root.destroy instead of top2.destroy
        self.button2.pack(side=LEFT, padx=20)

        self.MoreMsg = Button(frame2, text="Click This Other Me", fg="blue", command=self.write_message2, font=("arial", 16, "bold"))
        self.MoreMsg.pack(side=LEFT)

        self.button2.configure(height=2, width=20)
        self.MoreMsg.configure(height=2, width=20)

        self.anotherLabel = Label(frame2, text="This msg shows on the screen.", font=("arial", 14, "bold"), width=32, padx=10)  # created but not packed

    def write_message2(self):
        print("You have moved on to activity on Screen Number 2")  # sent to standard out & will duplicate multiple presses- not sent to GUI
